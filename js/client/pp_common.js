...
function responsiveStyle() {
	...
	if (windowWidth < 1025) {
		...
		// 기본 설정
		$('body').removeClass('scrolly on');
		$(".btn_admin").insertBefore($(".btn_log"));
		$('.gnb_1depth').removeClass('on');
		$('.gnb_2depth').removeClass('on');
		$('.gnb_2depth').removeAttr('style');
		$('.gnb_3depth').removeAttr('style');
...
function searchDetail() {
	...
	clickDetail.on('click', function(){
		$(this).toggleClass('on');
		$(this).parents('.search_inner').next('.search_detail').slideToggle();
	});
}
...
//	var swiper = new Swiper('.mainevent_wrap .swiper-container', {
//		slidesPerView: 1,
//		spaceBetween: 0,
//		navigation: {
//			nextEl: '.mainevent_wrap .swiper_next',
//			prevEl: '.mainevent_wrap .swiper_prev',
//		}
//	});
//}
